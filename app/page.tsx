  ...
  <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 32 }}>
      {[
        ...
        { label: "Active Quiz", value: activeDate || "—", accent: activeDate ? T.green : T.muted },
        { label: "Players Today", value: todayLb.length },
        { label: "Top Score", value: todayLb[0] ? `${todayLb[0].score}/${todayLb[0].total}` : "—" }
      ].map((s, i) => (
        <div key={i} style={{ background: T.panel, border: `1px solid ${T.panelBorder}`, borderRadius: 12, padding: 16 }}>
          <HUD label={s.label} value={s.value} accent={(s as any).accent} />
        </div>
      ))}
    </div>
    ...
    {tab === "generate" && (
      <div style={{ animation: "fadeIn 0.3s ease" }}>
        ...
        <div style={{ background: T.panel, border: `1px solid ${T.panelBorder}`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: T.muted, fontFamily: T.mono, letterSpacing: 2, marginBottom: 10 }}>PASTE QUESTIONS JSON</div>
          <Textarea
            placeholder='Paste JSON array here: [{"question":"...","options":["A","B","C","D"],"answer":0,"explanation":"..."}]'
            style={{ minHeight: 140 }}
            onChange={(e: any) => {
              try {
                const parsed = JSON.parse(e.target.value);
                if (Array.isArray(parsed) && parsed.length > 0) { setQuestions(parsed); setErr(""); }
              } catch {}
            }}
          />
          {err && <div style={{ color: T.red, fontSize: 12, fontFamily: T.mono, marginTop: 6 }}>{err}</div>}
        </div>

        {questions.length > 0 && <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ fontWeight: 700 }}>{questions.length} questions loaded</span><Tag color={T.green}>READY</Tag></div>
...
// ── PLAYER ───────────────────────────────────────────────
function PlayerApp() {
const [screen, setScreen] = useState(“home”); // home | name | quiz | result | leaderboard
const [quizzes, setQuizzes] = useState<Record<string, any[]>>({});
const [leaderboard, setLeaderboard] = useState<any[]>([]);
...
function selectQuiz(date: string) { setSelectedDate(date); setScreen(“name”); setErr(””); }
...
<div style={{ fontFamily: T.mono, fontSize: 11, color: T.accent, letterSpacing: 4, marginBottom: 8 }}>DAILY BRIEFING</div>
...
```
  {quizDates.length === 0 && (
    <div style={{ background: T.panel, border: `1px solid ${T.panelBorder}`, borderRadius: 14, padding: 24, textAlign: "center", color: T.muted, fontFamily: T.mono, fontSize: 13 }}>
      NO QUIZZES AVAILABLE — STAND BY
    </div>
  )}
  ...
  {quizDates.map(date => {
    ...
    const topScore = dateLb[0];
    return (
      <div key={date} onClick={() => selectQuiz(date)} style={{ background: T.panel, border: `1px solid ${isToday ? T.accent : T.panelBorder}`, borderRadius: 14, padding: 20, marginBottom: 12, cursor: "pointer", transition: "border-color 0.2s" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontFamily: T.mono, fontWeight: 700, fontSize: 16 }}>{date}</span>
        ...
        {dateLb.slice(0, 3).length > 0 && (
          <div style={{ display: "flex", gap: 8 }}>
            {dateLb.slice(0, 3).map((e: any, i: number) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "6px 10px", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                <span>{medal(i)}</span>
                <span style={{ fontWeight: 600 }}>{e.name}</span>
                <span style={{ color: T.accent, fontFamily: T.mono }}>{e.score}/{e.total}</span>
              </div>
            ))}
          </div>
        )}
...
<div style={{ fontFamily: T.mono, fontSize: 11, color: T.accent, letterSpacing: 4, marginBottom: 16 }}>QUIZ — {selectedDate}</div>
...
```
  {quizLb.length > 0 && (
    <div style={{ background: T.panel, border: `1px solid ${T.panelBorder}`, borderRadius: 12, padding: 16, marginBottom: 20 }}>
      <div style={{ fontSize: 11, color: T.muted, fontFamily: T.mono, letterSpacing: 2, marginBottom: 10 }}>CURRENT LEADERBOARD</div>
...
<div style={{ fontFamily: T.mono, fontSize: 11, color: T.accent, letterSpacing: 4, marginBottom: 8 }}>MISSION DEBRIEF — {selectedDate}</div>
...
<div style={{ fontSize: 11, fontFamily: T.mono, color: T.muted, letterSpacing: 2, marginBottom: 12 }}>QUIZ LEADERBOARD</div>
{quizLb.slice(0, 5).map((e: any, i: number) => (
