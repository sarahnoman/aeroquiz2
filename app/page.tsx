      ...
      <div style={{ padding: '12px 18px', borderTop: '1px solid rgba(56,189,248,0.08)', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={send} disabled={sending || !text.trim() || !name.trim()}
            style={{ background: text.trim() && name.trim() ? 'linear-gradient(135deg,rgba(56,189,248,0.25),rgba(56,189,248,0.1))' : 'rgba(255,255,255,0.04)', border: '1px solid '+(text.trim() && name.trim() ? '#38bdf8' : 'rgba(56,189,248,0.12)'), color: text.trim() && name.trim() ? '#38bdf8' : T.muted, borderRadius: 8, padding: '10px 18px', fontSize: 14, cursor: 'pointer', fontWeight: 700, transition: 'all 0.2s' }}>
            {sending ? '...' : '↑'}
          </button>
        </div>
      </div>
